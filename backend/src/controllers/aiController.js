...
export const analyzeSymptoms = async (req, res) => {
  ...
  try {
    // التعديل السحري: v1beta + gemini-1.5-flash
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: `أنت طبيب خبير. حلل الشكوى: "${symptoms}". رد بصيغة JSON فقط: {"possibleCondition": "...", "specialtyNeeded": "...", "advice": "..."}`
          }]
        }]
      })
    });
    ...
    res.status(500).json({ message: "حدث خطأ في النظام، حاول مرة أخرى" });
  }
};
